...
def videoplay(request):
    ...
    csv_file="C:/Users/jaink/Desktop/vigilAI IIIT/VigilAI_runningBackup/VigilAI/healthcare/healthcare/AIbase.csv"
    ...
    for row in reader:
            ...
            model_instance.photos = row[6]
            model_instance.fir = row[7]
            model_instance.save()
    data=[
        ['Name','Crime_Type','Location','Co-ordinates','Date','Time',"photo_loc","fir"]
    ]

    create_csv_file("C:/Users/jaink/Desktop/vigilAI IIIT/VigilAI_runningBackup/VigilAI/healthcare/healthcare/AIbase.csv",data)
    ...
    if request.method=='POST':
        ...
        for x in signup:
            if x.name==name and x.token==password:
                ...
                for x in video:
                    for y in x.photos:
                        if y=="'" and string!="," and string!="":
                            culprit[x.id].append(string)    
                            string=""
                        elif  y!="'" and y!="[" and y!="]":
                            string=string+y       
                return render(request,"play.html",{'video':video,'culprit':culprit,'firs':firs})   
